module.exports = function(config) {
    ...
    var all = function (anyAcademic, codAssignatura, codAula, classroomId, idp, s, next) {
        ...
        var getResumEstudiant = function (estudiant, next) {
            var tercer = mtercer.describe(config.indicadors.getValor(estudiant.tercer));
            config.async.parallel([
                function (next) {
                    mtercer.getResumActivitatAula(tercer.idp, classroomId, s, function (err, resum) {
                        tercer.resum = resum;
                        return next();
                    });
                },
                function (next) {
                    mtercer.getFitxa(tercer.idp, idp, s, function (err, url) {
                        tercer.fitxa = url;
                        return next();
                    });
                }

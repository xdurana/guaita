module.exports = function(app, config) {
    ...
    app.get('/app/guaita/plana', config.user.admin, function (req, res, next) {
        ...
        return assignatures.byidpdocent(s, idp, perfil, function (err, result) {
            ...
            return res.render('planaaules/consultor.html', result);
        });
    });
     ...
     * @returns {*}
     */
    app.get('/app/guaita/plana/:anyAcademic/:codAssignatura/:domainId/aules', config.user.authorize, function(req, res, next) {
        ...
        return aules.all(anyAcademic, codAssignatura, domainId, idp, s, perfil, function (err, result) {
            if (err) return next(err);
            return res.json({
                "id" : "03_515",

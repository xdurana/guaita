module.exports = function(app, config) {
    ...
    app.get('/app/guaita/assignatures/:anyAcademic/:codAssignatura/materials', config.user.authorize, function(req, res, next) {
        ...
        var domainIdSala = req.query.domainIdSala;
        ...
        config.services.mymat.listMaterialsAjax(s, codAssignatura, userId, function (err, object) {
            ...
            materials.forEach(function(material) {
                material.formats.forEach(function(format) {
                    if (format.url) {
                        format.url = format.url.replace("domainId=&", "domainId=" + domainIdSala + "&");
                    }
                });
            });
    ...
    /**
     * Fonts d'una assignatura (AULACA)
     * @param req
     * @param res
     * @param next
     */
    app.get('/app/guaita_/assignatures/:anyAcademic/:codAssignatura/fonts', config.user.authorize, function(req, res, next) {

        var s = req.query.s;
        var codAssignatura = req.params.codAssignatura;
        var anyAcademic = req.params.anyAcademic;

        config.services.aulaca.getFontsInformacioAssignatura(anyAcademic, codAssignatura, s, function (err, recursos) {
            res.json(recursos);
        });
    });

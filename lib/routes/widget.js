...
exports.two = function(anyAcademic, codAssignatura, subjectId, codAula, classroomId, domainCode, idp, libs, up_maximized, s, next) {
    ...
    var setEina = function(eina) {
        return {
            viewItemsUrl: eina.viewItemsUrl,
            nom: eina.translatedDescription,
            resourceId: eina.resourceId,
            idTipoLink: eina.idTipoLink,
            num_msg_pendents_class = eina.newItems > 0 ? 'nous' : 'nous cap',
            num_msg_pendents: eina.newItems,
            num_msg_totals: eina.totalItems
        }
    }
    ...
    ws.aulaca.getWidgetAula(subjectId, classroomId, s, function(err, result) {
        ...
        struct.nomAssignatura = result.widget.classroom.widgetTitle;
        struct.color = result.color;
        struct.urlAula = result.classroomAccessUrl;

        struct.consultor = null;
        for (ru in result.referenceUsers) {
            if (ru.userTypeId === 'CONSULTOR') {
                struct.consultor = {
                    fitxa: ru.userResume,
        ...
        struct.eines = [];
        for (eina in result.classroomResources) {
            struct.eines.push(setEina(eina));
        }
        ...
        if (result.currentActivity) {
            struct.activa = {
                link: '', //TODO
                name: result.currentActivity.name,
                startDateStr: result.currentActivity.startDateStr,
                deliveryDateStr: result.currentActivity.deliveryDateStr,
                eines: []
            };
            for (eina in result.currentActivity.resources) {
                struct.activa.push(setEina(eina));
            }
        }

        struct.urlAvaluacio = result.assessmentSystemAccessUrl;
        struct.connectats = result.connectedStudentsAccessUrl;

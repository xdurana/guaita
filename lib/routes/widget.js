...
exports.two = function(anyAcademic, codAssignatura, subjectId, codAula, classroomId, domainCode, idp, libs, up_maximized, s, next) {
    ...
    var setEina = function(eina) {
        ...
        e.num_msg_pendents = eina.newItems;
        e.num_msg_totals = eina.totalItems;
        e.num_msg_pendents_class = eina.newItems > 0 ? 'nous' : 'nous cap';
        return e;
    }
    ...
    ws.aulaca.getWidgetAula(subjectId, classroomId, s, function(err, result) {
        ...
        if (result.widget.currentActivity) {
            ...
            result.widget.currentActivity.resources.forEach(function(eina) {
                struct.activa.eines.push(setEina(eina));
            });
        }

        struct.urlAvaluacio = result.widget.assessmentSystemAccessUrl;
        struct.connectats = result.widget.connectedStudentsAccessUrl;

...
exports.two = function(anyAcademic, codAssignatura, subjectId, codAula, classroomId, domainCode, idp, libs, up_maximized, s, next) {
    ...
    var setEina = function(eina, docent, indicadors) {
        ...
        e.num_msg_pendents = Math.max(indicadors.resource.newItems, 0);
        e.num_msg_totals = indicadors.resource.totalItems;
        struct.missatgesPendents += e.num_msg_pendents;
        e.num_msg_pendents_class = e.num_msg_pendents > 0 ? 'nous' : 'nous cap';
        e.mostrar = (eina.visible == 0 || eina.visible == 1 && docent);
        return e;
    }

    var getIndicadorsEines = function(eina, next) {
        ws.aulaca.getIndicadorsEines(subjectId, classroomId, eina.resourceId, s, function(err, result) {
            struct.eines.push(setEina(eina, struct.docent, result));
            return next();
        });
    }
    ...
    ws.aulaca.isDocent(s, idp, subjectId, function(err, result) {
        ...
        ws.aulaca.getWidgetAula(subjectId, classroomId, s, function(err, result) {
            ...
            struct.urlAula = aules.getLinkAula(s, struct.isAulaca, subjectId, classroomId, domainCode);
            struct.urlAvaluacio = result.widget.assessmentSystemAccessUrl;
            ...
            struct.activa = null;
            if (result.widget.currentActivity) {
                struct.activa = {
                    link: '', //TODO
            ...
            struct.eines = [];
            async.each(result.widget.classroomResources, getIndicadorsEines, function(err) {
                return next(null, struct);
            });
        });
    });

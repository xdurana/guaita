...
var build = function(classrooms, idp, s, next) {
    ...
    var mostraAula = function(aula, next) {
        async.parallel([
            function(next) {
                widget.two(aula.anyAcademic, aula.codiAssignatura, aula.domainFatherId, aula.numeralAula, aula.domainId, aula.domainCode.slice(0, -3), idp, [], false, s, function(err, result) {
                    if (err) return next(err);
                    aula.widget = result;
                    return next();
                });

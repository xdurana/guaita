 ...
 * @param next
 */
exports.aula = function(anyAcademic, codAssignatura, codAula, classroomId, dies, s, next) {
    var ordre = 0;
    var estudiants = [];
    ws.rac.getEstudiantsPerAula(anyAcademic, codAssignatura, codAula, function(err, result) {
        if (err) return next(err);
        ...
        exports.ultimaActivitatEntregada(anyAcademic, codAssignatura, codAula, function(err, result) {
            ordre = result;
            async.each(estudiants, informacio, function(err) {
        ...
        async.parallel([
            function (next) {
                ws.lrs.byidpandclassroomlast(tercer.idp, classroomId, s, function(err, result) {
                    if (err) return next(err);
                    estudiant.diesquefa = result && indicadors.getValor(result.value) ? moment().diff(moment(indicadors.getValor(result.value).stored), 'days') : 100;
                    if (estudiant.diesquefa > dies) {
                        estudiant.ultimaConnexio = indicadors.getUltimaConnexio(result);
                    }
                    return next();
                });
            },
            function (next) {
                ws.rac.getActivitatsByEstudiantAulaOrdre(anyAcademic, codAssignatura, codAula, ordre, estudiant.numExpedient, function(err, result) {

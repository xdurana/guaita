 ...
 * @param next
 */
exports.estudiantsConnectatsFaMesDeNDies = function(anyAcademic, codAssignatura, codAula, classroomId, dies, s, next) {
    var estudiants = [];
    ws.rac.getEstudiantsPerAula(anyAcademic, codAssignatura, codAula, function(err, result) {
        if (err) return next(err);
        result.out = result.out || {};
        result.out.EstudiantAulaVO = result.out.EstudiantAulaVO || [];
        async.each(result.out.EstudiantAulaVO, getUltimaConnexioAula, function(err) {
            if (err) return next(err);
            return next(null, estudiants);
        });
    });
    ...
    /**
     * Última connexió d'un estudiant a l'aula
     * @param estudiant
     * @param next
     */
    var getUltimaConnexioAula = function(estudiant, next) {
        var tercer = new Tercer(indicadors.getValor(estudiant.tercer));
        ws.lrs.byidpandclassroomlast(tercer.idp, classroomId, s, function(err, result) {
            if (err) return next(err);
            tercer.diesquefa = result && indicadors.getValor(result.value) ? moment().diff(moment(indicadors.getValor(result.value).stored), 'days') : 100;
            if (tercer.diesquefa > dies) {
                tercer.ultimaConnexio = indicadors.getUltimaConnexio(result);
                estudiants.push(tercer);
            }
            return next();
        });
